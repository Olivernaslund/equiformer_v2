...
class ChemicalShiftHead(nn.Module):
    """
    Maps ℓ=0 node embeddings + atom type to scalar chemical shift per atom.
    
    Args:
        in_channels: number of ℓ=0 channels from transformer
        num_atom_types: number of unique atom types
        atom_embed_dim: dimension of atom embedding (default 16)
    """
    def __init__(self, in_channels, num_atom_types, atom_embed_dim=16):
        super().__init__()
        self.atom_embedding = nn.Embedding(num_atom_types, atom_embed_dim)
        self.linear = nn.Linear(in_channels + atom_embed_dim, 1)

        # initialize atom embedding small
        nn.init.uniform_(self.atom_embedding.weight, -0.01, 0.01)
        
    def forward(self, l0_features, atomic_numbers):
        """
        l0_features: Tensor of shape [n_atoms, in_channels] (ℓ=0 features)
        atomic_numbers: Tensor of shape [n_atoms] with integer atom type indices
        """
        atom_emb = self.atom_embedding(atomic_numbers)
        x = torch.cat([l0_features, atom_emb], dim=-1)
        pred = self.linear(x).squeeze(-1)  # shape: [n_atoms]
        return pred

@registry.register_model("equiformer_v2")
class EquiformerV2_NMRV2(BaseModel):
    """
    ...
    def __init__(
        ...
        # NMR shielding head
        self.nmr_head = ChemicalShiftHead(self.sphere_channels, self.max_num_elements, atom_embed_dim=16)
    ...
    def forward(self, data):
        ...
        nmr_pred = self.nmr_head(x.embedding[:, 0, :], atomic_numbers)
